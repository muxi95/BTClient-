 ...
 * @author Richx
 */
public class LBSQLiteDatabasePool {
    /**
     * 连接池的初始大小
     */
     ...
     * 升级时监听器
     */
    private OnSQLiteUpdateListener mOnSQLiteUpgradeListener;

    private Boolean isWrite;

    private static HashMap<String, LBSQLiteDatabasePool> poolMap = new HashMap<String, LBSQLiteDatabasePool>();

    /**
     * 单例模式
     *
     * @param context 上下文
     * @param params  数据库配置参数
     * @param isWrite 打开数据库如果是 isWrite为true,则磁盘满时抛出错误
     */
    public synchronized static LBSQLiteDatabasePool getInstance(Context context, TADBParams params, Boolean isWrite) {
        String dbName = params.getDbName().trim();
        LBSQLiteDatabasePool pool = poolMap.get(dbName);
        if (pool == null) {
            pool = new LBSQLiteDatabasePool(context, params, isWrite);
     ...
     * @param isWrite 打开数据库如果是 isWrite为true,则磁盘满时抛出错误
     */
    public LBSQLiteDatabasePool(Context context, TADBParams params, Boolean isWrite) {
     ...
     * 设置升级的的监听器
     *
     * @param upgradeListener 监听器
     */
    public void setOnSQLiteUpdateListener(OnSQLiteUpdateListener upgradeListener) {
        this.mOnSQLiteUpgradeListener = upgradeListener;
    }
     ...
     *
     * @param numSQLiteDatabase 要创建的数据库连接的数目
     */
     ...
     */
    private LBSQLiteDatabase newSQLiteDatabase() {
        // 创建一个数据库连接
        LBSQLiteDatabase sqliteDatabase = new LBSQLiteDatabase(context, params, mOnSQLiteUpgradeListener);
        sqliteDatabase.openDatabase(isWrite);
        // 返回创建的新的数据库连接
     ...
     */
    public synchronized LBSQLiteDatabase getSQLiteDatabase() {
        // 确保连接池己被创建
        if (pSQLiteDatabases == null) {
            // 连接池还没创建，则返回 null
            return null;
        ...
        // 获得一个可用的数据库连接
        LBSQLiteDatabase sqliteDatabase = getFreeSQLiteDatabase();
     ...
     * @return 返回一个可用的数据库连接
     */
    private LBSQLiteDatabase getFreeSQLiteDatabase() {
        // 从连接池中获得一个可用的数据库连接
        LBSQLiteDatabase sqLiteDatabase = findFreeSQLiteDatabase();
     ...
     * @return 返回一个可用的数据库连接
     */
    private LBSQLiteDatabase findFreeSQLiteDatabase() {
        LBSQLiteDatabase sqliteDatabase = null;
        PooledSQLiteDatabase pSQLiteDatabase;
        // 获得连接池向量中所有的对象
     ...
     *
     * @param sqliteDatabase 需要测试的数据库连接
     * @return 返回 true 表示此连接可用， false 表示不可用
     */
    private boolean testSQLiteDatabase(LBSQLiteDatabase sqliteDatabase) {
        if (sqliteDatabase != null) {
     ...
     */
    public void releaseSQLiteDatabase(LBSQLiteDatabase sqLiteDatabase) {
        // 确保连接池存在，如果连接没有创建（不存在），直接返回
        if (pSQLiteDatabases == null) {
            ALog.i(" 连接池不存在，无法返回此连接到连接池中 !");
            return;
        }
        PooledSQLiteDatabase pSqLiteDatabase;
     ...
     */
    private void closeSQLiteDatabase(LBSQLiteDatabase sqlLiteDatabase) {
    ...
    class PooledSQLiteDatabase {
        ...
        public PooledSQLiteDatabase(LBSQLiteDatabase sqliteDatabase) {
            this.sqliteDatabase = sqliteDatabase;
        }

        /** 返回此对象中的连接 */
        public LBSQLiteDatabase getSqliteDatabase() {
            return sqliteDatabase;
        }

        /** 设置此对象的，连接 */
        public void setSqliteDatabase(LBSQLiteDatabase sqliteDatabase) {
            this.sqliteDatabase = sqliteDatabase;
        }
